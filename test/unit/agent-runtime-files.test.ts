...
test("child-process runs maintain per-agent status, events, and output files", async () => {
	...
	try {
		...
		assert.match(status.content[0]!.text, /status=.*status\.json/);

		const loaded = await handleTeamTool({ action: "api", runId, config: { operation: "list-agents" } }, { cwd });
		const agents = JSON.parse(loaded.content[0]!.text);
		...
		assert.equal(JSON.parse(statusApi.content[0]!.text).taskId, first.taskId);
		const eventsApi = await handleTeamTool({ action: "api", runId, config: { operation: "read-agent-events", agentId: first.taskId } }, { cwd });
		assert.match(eventsApi.content[0]!.text, /message_end/);
		...
		assert.match(transcriptApi.content[0]!.text, /Mock JSON success/);
